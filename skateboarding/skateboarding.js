...
const ctxbg = background.getContext('2d'); /* layer for background info and grid*/
...
const displayMessage = function(text, duration=2000,
...
function printMouse(objc) {
  const textSize0 = Math.max(0, 3 - parseInt(Math.abs(objc.x*10)).
      toString().length);
  const textSize1 = Math.max(0, 3 - parseInt(Math.abs(objc.y*10)).
      toString().length);
...
const gameover = function(text) {
  ...
  const ginfo = insertedText + 'Gameover! <br>Joy: ' +
...
const saveGameLocal = function() {
  ...
  const filename = 'skateboarding_'+(dt.getFullYear() + 1).toString()+'_'+
...
function mirrorPoint(p, p0, p1) {
  ...
  const y = parseFloat(b) * parseFloat(p.x - p0.x) -
...
function collide(obj, lineStart, lineEnd, dotlinedis) {
  obj.hit = 40;
  const mirroredVector = mirrorVector({x: obj.vx, y: obj.vy},
      lineStart, lineEnd);
  const intensity = Math.sqrt((mirroredVector.x-obj.vx) *
...
function collision(obj) {
  ...
  if (closestNode.j>0 && closestNode.i>=0) {
    ...
    const llen = getDistance(trails[closestNode.i][closestNode.j-1],
        trails[closestNode.i][closestNode.j]);
...
$( document ).ready(function() {
  const spaceBelow = $(window).height() -
