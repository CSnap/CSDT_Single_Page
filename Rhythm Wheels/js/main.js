...
// Classes, Ids, etc for GUI manipulation
let constants = {
...
RhythmWheels.prototype.compileAudio = function (toExport) {
  ...
  for (let i = 0; i < this.wc.wheelCount; i++) {
    let sequenceTime =
      (this.wc.wheels[i].loopCount * this.wc.wheels[i].nodeCount * 60.0) /
      globals.bpm;
...
RhythmWheels.prototype.recordedAudioBufferFill = function (
  ...
  let recordWheelBuffer = myself.ac.createBuffer(
    1,
...
RhythmWheels.prototype.mp3Export = function () {
  ...
  let recordedAudioMax =
    myself.wc.rapWheel.loopCount * globals.recordAudioDuration;
...
Wheel.prototype.createRepeatCount = function (wheelContainer) {
  ...
  loopCountControl.classList.add(
    "wheel_repeat_input",
    "form-control",
    "form-control-sm"
  );
  //   loopCountControl.style["width"] = "2em";
  loopCountControl.type = "number";
