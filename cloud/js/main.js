...
$(`#${cloudUI.projectNameField}`).on('keyup', () => {
    $(`#${cloudUI.projectNameField}`).attr('value', $(`#${cloudUI.projectNameField}`).val());
...
Cloud.prototype.updateProjectListing = function (pullFromAPI = true) {
    ...
    if (globals.isLoggedIn) {
        ...
        // Determine if we are clearing the project list, or just prepending to it.
        $(`#${cloudUI.loadProjectList}`).html(pullFromAPI ? '' : $(`#${cloudUI.loadProjectList}`).html());

        // Show loader and load message
        $(`#${cloudUI.loadProjectLoader}`).addClass(currentlyFetching ? 'd-flex' : '');
        $(`#${cloudUI.loadProjectLoader}`).removeClass(currentlyFetching ? 'd-none' : '');
        ...
        function successfullyFetchedProjects(projects) {
            ...
            $(`#${cloudUI.loadProjectLoader}`).addClass(!currentlyFetching ? 'd-none' : '');
            $(`#${cloudUI.loadProjectLoader}`).removeClass(!currentlyFetching ? 'd-flex' : '');
            $(`#${cloudUI.loadProjectMsg}`).attr('hidden', !currentlyFetching);
        ...
        function errorFetchingProjects(err) {
            console.error('Note to Developer: Failed to fetch current user\'s projects');
            console.error(`Error Message: ${JSON.stringify(err)}`);
        }

        // Retrieve the projects
        myself.listProject(globals.currentUserID, successfullyFetchedProjects, errorFetchingProjects);
...
/** submitSignInRequest: Attached to the sign in button (as part of the sign in prompt). This actually gathers the user's credendials, attempt
...
Cloud.prototype.submitSignInRequest = function () {
    ...
    let failedGetUser = function (err) {
        ...
        $(`#${cloudUI.usernameField}, #${cloudUI.passwordField}`).on('keyup', (e) => {
    ...
    let successfulPreImageSave = function (data) {
        ...
        let successfulPreProjectSave = function (data) {
            ...
            if (globals.isNewProject) {
                ...
                myself.updateProject(globals.currentProjectID, globals.currentProjectName,
        ...
        myself.saveFile(projectForm, successfulPreProjectSave, failedPreProjectSave);
    }
