...
function drawFlag(ox, oy) {
	...
	ctx.lineWidth = (2 * scale).toString();
    ctx.strokeStyle = 'black';
	ctx.fillStyle = '#de0000';
	ctx.fillRect(cxy.x, cxy.y - parseInt(20*scale), 
	parseInt(20*scale), parseInt(15*scale));
    ctx.strokeRect(cxy.x, cxy.y - parseInt(20*scale), 
    parseInt(20*scale), parseInt(15*scale));
	ctx.beginPath();
	ctx.moveTo(cxy.x, cxy.y - parseInt(20*scale));
	ctx.lineTo(cxy.x, cxy.y + parseInt(10*scale));
	ctx.stroke();
	*/
	let cxy = objToCanvas({x: ox, y: oy});
...
function drawPlayer(obj) {
	...
	if (debugMode) {
		...
		ctx.lineTo(obj.x + 8*speedVectorContant*obj.vx, obj.y + 8*speedVectorContant*obj.vy);
		ctx.lineWidth = 1;
...
function drawMan(obj, ox, oy) {
    ...
    if (canvas2.getContext) {
		...
		if (obj.valid && obj.hit == 0) {
			...
			ctx2.drawImage(skateboardhitpng, -width2 / 2, -height2 / 2, width2, height2);
			obj.hit -= 1;
		} else {
			ctx2.drawImage(skateboardfailpng, -width2 / 2, -height2 / 2, width2, height2);
		}
		ctx2.rotate(-obj.angle*Math.PI/180);
		ctx2.translate(-x2, -y2);
    }
...
function drawTrail(oneTrail) {
    if (oneTrail.length>0) {
        ctx.beginPath();
		let cxy = objToCanvas({x: parseInt(oneTrail[0].x), y: parseInt(oneTrail[0].y)});
        ctx.moveTo(cxy.x, 2+cxy.y);
        for (let i=1; i<oneTrail.length; i++) {
			cxy = objToCanvas({x: parseInt(oneTrail[i].x), y: parseInt(oneTrail[i].y)});
        ...
        ctx.beginPath();
		cxy = objToCanvas({x: parseInt(oneTrail[0].x), y: parseInt(oneTrail[0].y)});
...
function drawGrid() {
	...
	*/
    ...
    for (let i = Math.round(o00.x/gridSize); i <= Math.round(oWH.x/gridSize); i++) {
    ...
    for (let i = Math.round(oWH.y/gridSize); i <= Math.round(o00.y/gridSize); i++) {
...
function updateScreen() {
	...
	*/
	if (!paused) {
		CanvasOffsetX = W * 0.5 - parseInt(scale * (skateBoarder.x + 1 * skateBoarder.vx));
		CanvasOffsetY = H * 0.5 - parseInt(scale * (skateBoarder.y + 1 * skateBoarder.vy));
		drawNewGrid = true;
	}
...
function drawGraph() {
	...
	for (let id = 0; id < equation.length; id++) {
		...
		|| equation[id] == '/' || equation[id] == ')' || equation[id] == '(') {
			equation = equation.slice(0, id) + ' ' + equation[id] + ' ' + equation.slice(id+1, equation.length);
			id += 2;
		}
	}
...
function mirrorPoint(p, p0, p1) {
    ...
    let y = parseFloat(b) * parseFloat(p.x - p0.x) - parseFloat(a) * parseFloat(parseFloat(p.y) - p0.y) + parseFloat(p0.y);
    return {x: x, y: y};
}
 /**
     * @brief Reflect vector p along line through points p0 and p1
...
function collide(obj, lineStart, lineEnd) {
	obj.hit = 10;
	let mirroredVector = mirrorVector({x: obj.vx, y: obj.vy}, lineStart, lineEnd);
	let intensity = Math.sqrt((mirroredVector.x-obj.vx) * (mirroredVector.x-obj.vx) + (mirroredVector.y-obj.vy) * (mirroredVector.y-obj.vy));
    ...
    // obj.vx = obj.vx* (1 - 1 * vertLine.x / vertLen);
    // obj.vy = obj.vy* (1 - 1 * vertLine.y / vertLen);

	let newangle = (180 + Math.atan2(lineEnd.y-lineStart.y, lineEnd.x-lineStart.x) * 180 / Math.PI) % 180;
	...
	if (debugMode) {
		let vertStart={x: parseFloat(lineStart.x) + 2000*(lineEnd.y-lineStart.y), y: parseFloat(lineStart.y) - 2000*(lineEnd.x-lineStart.x)};
		let vertEnd={x: parseFloat(lineStart.x) - 2000*(lineEnd.y-lineStart.y), y: parseFloat(lineStart.y) + 2000*(lineEnd.x-lineStart.x)};
		ctx.beginPath();
        ...
        for (j=1; j<trails[i].length-1; j++) {
            ...
            if (closestNode.distance >= closestDistance &&
			    dotProduct(obj, {x: trails[i][j].x, y: trails[i][j].y},
						   obj, {x: obj.x + speedVectorContant * obj.vx, y: obj.y + speedVectorContant * obj.vy}) >= 0) {
	...
	if (trails.length > 0){
		...
		for (let offset=0; offset<2+parseInt(speed)/100; offset++){
			if (intersect(thisTrail[Math.max(0, pos-offset-1)], 
						  ...
						  {x:obj.x + speedVectorContant * obj.vx, y:obj.y + speedVectorContant * obj.vy})){
				collide(obj, thisTrail[Math.max(0, pos-offset-1)], thisTrail[Math.max(0, pos-offset)]);
				return;
			}
			if (intersect(thisTrail[Math.min(thisTrail.length-1, pos+offset)], 
						  thisTrail[Math.min(thisTrail.length-1, pos+offset+1)], obj, 
						  {x:obj.x + speedVectorContant * obj.vx, y:obj.y + speedVectorContant * obj.vy})){
				collide(obj, thisTrail[Math.min(thisTrail.length-1, pos+offset)], thisTrail[Math.min(thisTrail.length-1, pos+offset+1)]);
				return;
			}
		}
	}
	*/
	if ((getDistance(closestNode, obj) < obj.collisionR) && obj.vy < 0) {
		if (j+1>=trails[closestNode.i].length) {
			collide(obj, trails[closestNode.i][closestNode.j-1], trails[closestNode.i][closestNode.j]);
		} else {
			collide(obj, trails[closestNode.i][closestNode.j], trails[closestNode.i][closestNode.j+1]);
		}
		return;
	}
}
