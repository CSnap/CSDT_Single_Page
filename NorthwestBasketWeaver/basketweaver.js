...
class Wampum {
    ...
    stamp(point = this._initPoint, color = this._initColor) {
        ...
        if (navajoKnots) {
            this._ctx.ellipse(point.x, point.y, beadSize, beadSize / 2, -0.08 * Math.PI, 0, 2 * Math.PI, false);
            this._ctx.ellipse(point.x + beadSize, point.y + beadSize, beadSize, beadSize / 2, -0.08 * Math.PI, 0, 2 * Math.PI, false);
            this._ctx.ellipse(point.x - beadSize, point.y - beadSize, beadSize, beadSize / 2, -0.08 * Math.PI, 0, 2 * Math.PI, false);
            this._ctx.ellipse(point.x + beadSize, point.y - beadSize, beadSize, beadSize / 2, -0.08 * Math.PI, 0, 2 * Math.PI, false);
            this._ctx.ellipse(point.x - beadSize, point.y + beadSize, beadSize, beadSize / 2, -0.08 * Math.PI, 0, 2 * Math.PI, false);
            ...
            // Create gradient
            let grd = this._ctx.createLinearGradient(point.x - (beadSize), point.y - beadSize, point.x - (beadSize), (point.y - (beadSize)) + scale);
            ...
            if (((point.x / scale) % 2 == 0 && (point.y / scale) % 2 == 0) || ((point.x / scale) % 2 != 0 && (point.y / scale) % 2 != 0)) {
                this._ctx.fillRect(point.x - (basketSize), point.y - beadSize, scale - (basketSize * 2), scale);
            } else {
                this._ctx.fillRect(point.x - (beadSize), point.y - basketSize, scale, scale - (basketSize * 2));
            }
    ...
    triangleIteration() {
        ...
        let currentBead;
        let gradient = updateSpitter(this._initColor, this._iterColor, Math.ceil(cycles / steps));
        let inc = 0;
...
function loadFromJSON(text) {
    ...
    JSON.parse(text).forEach((obj) => {
        ...
        ++currBeadLength;
        beads[currBeadLength].setAdditionalParams(obj.endPoint, obj.topPoint, obj.rows, obj.iterColor, obj.linearRowLength, obj.linearPreNum,
